...
# Assign labels based on the new strategy
def assign_labels(x_ctr):
    # Sort CTR values and determine thresholds
    sorted_ctr = np.sort(x_ctr)
    n = len(sorted_ctr)

    # Define thresholds, ensuring they are integers
    top_20_percent_index = int(n * 0.8)  # Index for the top 20%
    worst_10_percent_index = int(n * 0.1)  # Index for the worst 10%

    # Initialize labels
    labels = np.zeros(n)

    # Assign -1 to the top 20% using integer indices
    labels[top_20_percent_index:] = -1

    # Assign 1 to the worst 10% using integer indices
    labels[:worst_10_percent_index] = 1

    return labels

# Get the new labels
y_labels = assign_labels(x_ctr)

y_negative = 1 - y_ctr  # Adjust based on your business logic
np.save('negative_recommendations.npy', y_negative)
...
# creatives_embeddings_dim=664
# creatives_embeddings_dim=1024 #1408 for google multimodal embeddings
creatives_embeddings_dim=1408 # Set to match the actual shape of X_creatives
audience_embeddings_dim=768
...
# Split data
X_creative_train, X_creative_val, X_audience_train, X_audience_val, y_train, y_val = train_test_split(
    X_creatives, X_audiences, y_labels, test_size=0.2, random_state=42
)
...
# Initialize Google Cloud Storage, Vertex AI, and Qdrant clients
storage_client = storage.Client()
endpoint = aiplatform.Endpoint('projects/the-story-teller-379307/locations/us-central1/endpoints/5218195324006301696')
bucket_name = os.getenv("GCS_BUCKET_NAME", "storyteller-vertex-ai")
qdrant_client = QdrantClient(url="https://staging-vector-db-storyteller-staging.app-staging.thestoryteller.tech:443")
...
# Function to download media to GCS
def download_media_to_gcs(media_url: str, folder: str, file_name: str) -> str:
    bucket = storage_client.bucket(bucket_name)
    blob = bucket.blob(f"{folder}/{file_name}")

    if not blob.exists() or blob.size == 0:
        try:
            response = requests.get(media_url, stream=True)
            response.raise_for_status()
            blob.upload_from_string(response.content)
            print(f"Downloaded and uploaded {media_url} to GCS at {folder}/{file_name}")
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Failed to download {media_url}: {e}")
    else:
        print(f"File already exists in GCS: {folder}/{file_name}")
    return f"{folder}/{file_name}"

# Function to retrieve embeddings for different modalities
def get_embeddings(file_path, prefix, modality):
    instances = [{modality: [f"gs://{bucket_name}/{prefix}/{file_path}"]}]
    response = endpoint.predict(instances=instances)
    if response and response.predictions:
        embedding_array = np.array(response.predictions[0][modality][0])
        return embedding_array
    else:
        raise HTTPException(status_code=500, detail=f"No embeddings returned for {file_path}")

# Function to download and load the creative tower model from TrueFoundry
def load_creative_tower_model(org_id: str, platform_id: str):
    model_name = f"model:storyteller/two-tower-model/{org_id}_{platform_id}_creative_tower:8"
    print("Downloading creative tower model from TrueFoundry...")

    tf_client = get_client()
    try:
        model_version = tf_client.get_model_version_by_fqn(model_name)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Creative tower model not found: {e}")

    model_download_path = "model_downloads"
    os.makedirs(model_download_path, exist_ok=True)
    download_info = model_version.download(path=model_download_path, overwrite=True)
    model_path = download_info.model_dir

    creative_tower = TFSMLayer(model_path, call_endpoint='serving_default')
    print("Creative tower model loaded successfully.")
    return creative_tower

# Function to retrieve all audience embeddings from Qdrant and save them to an Excel file
def get_audience_embeddings(audience_collection):
    audience_embeddings = []
    audience_ids = []
    audience_metadata = []  # To store additional metadata
    try:
        offset = None
        while True:
            response, next_offset = qdrant_client.scroll(
                collection_name=audience_collection,
                limit=1000,
                offset=offset,
                with_vectors=True
            )

            embeddings_batch = [np.array(item.vector) for item in response]
            ids_batch = [item.id for item in response]
            metadata_batch = [item.payload for item in response]  # Assuming metadata is in 'payload'
            audience_embeddings.extend(embeddings_batch)
            audience_ids.extend(ids_batch)
            audience_metadata.extend(metadata_batch)

            if not next_offset:
                break
            offset = next_offset
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching audience embeddings: {e}")

    # Convert embeddings to lists for Excel storage
    embeddings_list = [embedding.tolist() if isinstance(embedding, np.ndarray) else embedding for embedding in audience_embeddings]

    # Save audience embeddings to a separate Excel file
    embeddings_df = pd.DataFrame({
        "id": audience_ids,
        "embedding": embeddings_list,  # Now safely converted to lists
        "metadata": audience_metadata  # Metadata should be in a serializable format like a dictionary
    })
    embeddings_df.to_excel("audience_embeddings.xlsx", index=False)

    return np.array(audience_embeddings), audience_ids

# Endpoint to get embeddings and perform similarity search using ScaNN
@app.post("/find-similar-audiences")
async def find_similar_audiences(request: EmbeddingRequest):
    org_id = request.org_id
    platform_id = request.platform_id
    kpi_name = request.kpi_name
    media_url = request.media_asset_url

    # Define the audience collection name in Qdrant
    audience_collection = f"{org_id}_{platform_id}_{kpi_name}_audience_data"

    # Verify and load all audience embeddings, and save them to an Excel file
    audience_embeddings, audience_ids = get_audience_embeddings(audience_collection)

    # Determine file extension and folder in GCS
    ...
    # Determine GCS folder and download media if needed
    folder = image_folder if file_extension in ['jpg', 'png'] else video_folder if file_extension in ['mp4', 'mov'] else None
    if not folder:
        raise HTTPException(status_code=400, detail=f"Unsupported media type: {file_extension}")

    # Download media to GCS
    gcs_blob_name = download_media_to_gcs(media_url, folder, file_name)

    # Retrieve embeddings for available modalities
    embeddings_list = []
    if file_extension in ['jpg', 'png']:
        image_embedding = get_embeddings(file_name, image_folder, 'vision')
        embeddings_list.append(image_embedding)
    elif file_extension in ['mp4', 'mov']:
        video_embedding = get_embeddings(file_name, video_folder, 'video')
        embeddings_list.append(video_embedding)

        # Check for audio extracted from video
        audio_file = file_name.replace(file_extension, "wav")
        audio_blob_name = download_media_to_gcs(media_url, audio_folder, audio_file)
        audio_blob = storage_client.bucket(bucket_name).blob(audio_blob_name)
        if audio_blob.exists():
            audio_embedding = get_embeddings(audio_file, audio_folder, 'audio')
            embeddings_list.append(audio_embedding)

    # Average embeddings if they exist
    if embeddings_list:
        averaged_embedding = np.mean(embeddings_list, axis=0)
    else:
        raise HTTPException(status_code=500, detail="No embeddings found for the media asset.")

    # Load the creative tower model and transform the averaged embedding
    creative_tower = load_creative_tower_model(org_id, platform_id)
    transformed_embedding = creative_tower(tf.convert_to_tensor([averaged_embedding], dtype=tf.float32))['output_0'].numpy()[0]

    # Save the transformed embedding to a separate Excel file for verification
    transformed_df = pd.DataFrame({"transformed_embedding": [transformed_embedding.tolist()]})
    transformed_df.to_excel("transformed_embedding.xlsx", index=False)

    # Build the ScaNN searcher
    searcher = scann.scann_ops_pybind.builder(audience_embeddings, 10, "dot_product") \
        .tree(num_leaves=168, num_leaves_to_search=100, training_sample_size=155).score_ah(2, anisotropic_quantization_threshold=0.2) \
        .reorder(100) \
        .build()

    # Perform nearest neighbor search using ScaNN
    indices, distances = searcher.search(transformed_embedding)
    similar_audiences = [
        {"id": audience_ids[idx], "distance": float(distance)}
        for idx, distance in zip(indices, distances)
    ]

    return {
        "media_asset_url": media_url,
